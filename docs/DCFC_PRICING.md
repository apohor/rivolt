...
## Rates (USD/kWh, US deployments, mid-2025)
...
| Tesla Supercharger (non-Tesla) | 0.55 | 0.40 | Supercharging Membership — $12.99/mo | [tesla.com/support/charging/supercharger/non-tesla](https://www.tesla.com/support/charging/supercharger/non-tesla) |
| Rivian Adventure Network | 0.45 | — | Rivian owner only (no separate tier) | [rivian.com/support/article/rivian-adventure-network](https://rivian.com/support/article/rivian-adventure-network) |
| EVgo | 0.42 | 0.34 | Rewards+ — $6.99/mo | [evgo.com/charging-plans](https://www.evgo.com/charging-plans/) |
| Blink | 0.49 | 0.39 | Member — annual | [blinkcharging.com/pricing](https://www.blinkcharging.com/pricing) |
| bp pulse | 0.45 | 0.39 | Plus — $4/mo | [bppulsefleet.com](https://bppulsefleet.com/) |
| Shell Recharge | 0.43 | 0.40 | GO+ — free | [shellrecharge.com](https://shellrecharge.com/) |
| ChargePoint | 0.45 | — | Host-set (no central plan) | [chargepoint.com](https://www.chargepoint.com/) |
| Francis Energy | 0.40 | — | No public plan | [francisenergy.com](https://francisenergy.com/) |
| Ionna | 0.40 | — | Limited deployment; pricing TBD | [ionna.com](https://www.ionna.com/) |
| Flo | 0.35 | — | No US plan | [flo.com](https://www.flo.com/) |
| **Unmatched DCFC** | **0.46** | — | Fallthrough when name doesn't match | — |

> **Rivian Adventure Network** is Rivian-only by access — every
> Rivolt user is by definition a Rivian owner, so the "member" rate
> is auto-applied. The table shows the same number in both columns.
...
- **Regional variance is real.** EA in CA is closer to $0.56,
  EA in TX is $0.48. The flat number is wrong by ±10% in either
...
- **Time-of-day variance is real.** Tesla Supercharger pricing
  swings $0.20/kWh between off-peak and peak. We pick the mid-band.
...
- **Operator self-reporting via NREL is patchy.** The chargers
  ...
  use it per-stop — that's slice B (deferred). When we wire it,
  the per-stop quote can override the network default with the
...
- An operator publishes a rate change (EA's March 2023 hike from
  $0.43 → $0.48 was the canonical example).
- A new network reaches Rivolt's user base (Ionna, e.g.).
- Multiple Rivolt users report a meaningful gap between the
  table rate and their session receipts. **The long-term fix is
  slice C** — replacing the table with each user's rolling
  average from their own `charges` history. Until then the table
  is the bridge.
...
America - Pflugerville" → some new format) gets flagged on the
next CI run.
...
5. Run `go test ./internal/tripadvice/...` to make sure the
   ...
   guest > member and rates ∈ (0, $2/kWh).
