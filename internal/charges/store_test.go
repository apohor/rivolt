...
// CloseStaleOpenLive, and the janitor sweep. The list previously
// was "charging_*" minus charging_complete and charging_station_err,
// which incorrectly included transient terminal states like
// charging_user_stopped and charging_station_stopped — so
// resumeOpenCharge would reattach to a just-closed row and absorb
...
func TestIsOpenLiveFinalState(t *testing.T) {
	...
	terminal := []string{
		"charging_user_stopped",    // driver-initiated stop
		"charging_station_stopped", // station-initiated stop
		"charging_complete",
