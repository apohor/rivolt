...
```bash
make dev          # docker compose: Go on :8080 + Vite on :5173 (HMR proxies /api)
make dev-api      # Go server only (no docker), reads ./data
...
Every release bumps **two** files in lockstep — `deploy/helm/rivolt/Chart.yaml` here, then `apps/rivolt/app.yaml` in `rivolt-infra`. The chart version **must equal** a pushed git tag, otherwise ArgoCD fails with `unable to resolve 'vX.Y.Z' to a commit SHA`. Full procedure in `.cursor/rules/ship-it.mdc`. CI publishes `ghcr.io/apohor/rivolt:X.Y.Z` only on tag push, so push the tag explicitly (`git push origin vX.Y.Z`) — `--follow-tags` skips lightweight tags.
